...
class PPOAgent:
    def __init__(self, state_size, output, action_size, seed, hidden_layers, lr_policy, use_reset, device):
        #self.main_net = FullyConnectedNet(seed, 4, 64, 64).to(device)
        self.main_net = FullyConnectedNet(seed, state_size, hidden_layers, output).to(device)
        print("h")
